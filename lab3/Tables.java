...
public class Tables {
	...
	public String getTokken(int code){
		if (code < 0) {
			return "ERROR: Tokken doesn't exist";
		}
		if (code < 128) {
	...
	void printTreeNode(int level, TreeNode root) {
		...
		for (int i=0; i<root.kids.size(); i++) {
			printTreeNode(level + 2, root.get(i));
		}
	}
	void printTree() {
		System.out.println("\nTree:\n");
		...
		for (int i=0; i<treeRoot.kids.size(); i++) {
			printTreeNode(2, treeRoot.get(i));
		}
	}
	public void addCodeError(Lexeme buff, String message) {
		addError("Code Generator: Error (line "+buff.row+", column "+buff.pos +"): " + message);
	}
}
